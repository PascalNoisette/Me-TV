...
gboolean Mpeg::Stream::is_pid_used(guint pid) {
	...
	while (index < audio_streams.size() && !used) {
		if (pid==audio_streams[index].pid) { used = true; }
		else { ++index; }
	}
	index = 0;
	while (index < subtitle_streams.size() && !used) {
		if (pid==subtitle_streams[index].pid) { used = true; }
		else { ++index;
		}
	}
	return used;
...
void Mpeg::Stream::build_pat(guchar * buffer) {
	...
	// Program Map PID
	pmt_pid = 0xff;
	while (is_pid_used(pmt_pid)) { --pmt_pid; }
	buffer[0x11] = 0x03;
	buffer[0x12] = 0xe8;
	buffer[0x13] = 0xe0;
	buffer[0x14] = pmt_pid;
	...
	for (gint i=0x19; i < 188; ++i) { buffer[i]=0xff; }
}

void Mpeg::Stream::build_pmt(guchar * buffer) {
	int i, off=0;
	Mpeg::VideoStream video_stream;
	if (!video_streams.empty()) { video_stream = video_streams[0]; }
	buffer[0x00] = 0x47;
	buffer[0x01] = 0x40;
	buffer[0x02] = pmt_pid;
	...
	// needed stuffing bytes
	for (i = off + 5 ; i < 188 ; ++i) { buffer[i] = 0xff; }
}

void Mpeg::Stream::set_pmt_pid(Buffer const & buffer, guint service_id) {
	...
	if (pmt_pid == 0) { throw Exception(_("Failed to find Program Map PID for service")); }
}

void dump_descriptors(unsigned char const * buffer, int descriptors_loop_length) {
...
void Mpeg::Stream::parse_pms(Buffer const & buffer) {
	guchar const * desc = NULL;
	if (buffer[0] != 2) { throw Exception("Section is not a Program Mapping Section"); }
	guint offset = 8;
...
gboolean Mpeg::Stream::find_descriptor(guchar tag, unsigned char const * buf, int descriptors_loop_len,
	unsigned char const ** desc, int * desc_len) {
	while (descriptors_loop_len > 0) {
		guchar descriptor_tag = buf[0];
		guchar descriptor_len = buf[1] + 2;
		if (!descriptor_len) { break; }
...
Glib::ustring Mpeg::Stream::get_lang_desc(guchar const * b) {
	char c[4];
