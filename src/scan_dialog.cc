...
ScanDialog::ScanDialog(BaseObjectType * cobject, Glib::RefPtr<Gtk::Builder> const & builder)
...
void ScanDialog::import_channels_conf(Glib::ustring const & channels_conf_path) {
...
void ScanDialog::add_channel_row(Channel const & channel, guint signal_strength) {
...
// TODO: This assumes v3 files, but files are now v5. Need another parser.
void ScanDialog::load_initial_tuning_file(Glib::ustring const & initial_tuning_file_path) {
	...
	while (status == Glib::IO_STATUS_NORMAL) {
		...
		if (index == Glib::ustring::npos) { line.clear(); }
		else { line = line.substr(0, index + 1); }
	...
	for (auto const process_line: lines) {
		if (!process_line.empty()) {
			...
			if (Glib::str_has_prefix(process_line, "T ")) { process_terrestrial_line(process_line); }
			else if (Glib::str_has_prefix(process_line, "T2 ")) { g_debug("TODO: Fix HD channels"); } // TODO: Fix HD channels
			else if (Glib::str_has_prefix(process_line, "C ")) { process_cable_line(process_line); }
			else if (Glib::str_has_prefix(process_line, "S ")) { process_satellite_line(process_line); }
			else if (Glib::str_has_prefix(process_line, "A ")) { process_atsc_line(process_line); }
			else { throw Exception(_("Me TV cannot process a line in the initial tuning file")); }
		}
	}
}
...
void ScanDialog::add_auto_scan_range(fe_type_t frontend_type, Glib::ustring const & range) {
	if (range.empty()) {
