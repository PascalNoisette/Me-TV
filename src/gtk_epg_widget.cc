...
constexpr auto MINUTES_PER_COLUMN = 1;
constexpr auto COLUMNS_PER_HOUR	= (60 * MINUTES_PER_COLUMN);

GtkEpgWidget::GtkEpgWidget(BaseObjectType * cobject, Glib::RefPtr<Gtk::Builder> const & builder) :
...
void GtkEpgWidget::create_channel_row(Gtk::RadioButtonGroup& group, Channel& channel,
	...
	guint last_event_end_time = 0; // TODO: unused?
...
bool GtkEpgWidget::on_channel_button_press_event(GdkEventButton* event, guint channel_id) {
	if (event->type == GDK_BUTTON_PRESS && event->button == 3) {
		...
		if (stream_manager.is_recording(channel)) {
			get_the_application().stop_recording(channel);
		}
		else {
			get_the_application().start_recording(channel);
		}
		update_table();
	}
	return false;
