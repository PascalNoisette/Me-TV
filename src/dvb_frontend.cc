...
Frontend::Frontend(Adapter const & frontend_adapter, guint frontend_index)
	: adapter(frontend_adapter) {
	fd = -1;
	memset(&frontend_parameters, 0, sizeof(dvb_frontend_parameters));
...
void Frontend::tune_to(Transponder const & transponder, guint timeout) {
	g_message(_("Frontend::tune_to(%d)"), transponder.frontend_parameters.frequency);
	dvb_frontend_parameters parameters = transponder.frontend_parameters;
	...
	if(frontend_info.type == FE_QPSK) {
		...
		// Note that satellite_number is constant 0. Maybe this should be configurable? I don't fully understand
		// it's use, but it has something to do with being able to tune to different satellites.
		if (hi_band == 1) {
