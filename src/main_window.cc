...
void MainWindow::show_channels_dialog() {
	...
	if (dialog_result == Gtk::RESPONSE_OK) {
		...
		ChannelArray const & channels = channels_dialog.get_channels();
		stream_manager.stop();
...
void MainWindow::set_status_text(Glib::ustring const & text) {
...
void MainWindow::show_error(Glib::ustring const & message) {
