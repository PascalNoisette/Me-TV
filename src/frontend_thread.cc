...
constexpr auto READ_TIMEOUT = 10; /* In milliseconds */
...
void FrontendThread::run() {
	...
	while (!is_terminated()) {
		...
		try {
			if (::poll(pfds, 1, READ_TIMEOUT) < 0) { throw SystemException("Frontend poll failed"); }
...
void FrontendThread::setup_dvb(ChannelStream & channel_stream) {
	...
	Buffer buffer;
	Channel const & channel = channel_stream.channel;
...
Glib::ustring make_recording_filename(Channel & channel, Glib::ustring const & description) {
...
void FrontendThread::start_recording(Channel & channel, Glib::ustring const & description, gboolean scheduled) {
	stop();
...
void FrontendThread::stop_recording(Channel const & channel) {
	stop();
	// TODO: There must be an easier way…
	ChannelStreamList::iterator iterator = streams.begin();
	while (iterator != streams.end()) {
...
gboolean FrontendThread::is_recording() {
	for (auto const & stream: streams) {
		if (is_recording_stream(stream)) { return true; }
	}
	return false;
}
...
gboolean FrontendThread::is_display() {
	for (auto const & stream: streams) {
		if (stream->type == CHANNEL_STREAM_TYPE_DISPLAY) { return true; }
	}
	return false;
}
