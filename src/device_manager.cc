	...
	while (Gio::File::create_for_path(adapter_path)->query_exists()) {
		...
		while (Gio::File::create_for_path(frontend_path)->query_exists()) {
			if (devices.empty() || devices.find(frontend_path) != Glib::ustring::npos) {
				...
				try {
					frontend->open();
					if (!is_frontend_supported(*frontend)) { g_debug("Frontend not supported"); }
					else {
				...
				catch(...) { g_debug("Failed to load '%s'", frontend_path.c_str()); }
			}
			frontend_path = get_frontend_path(adapter_count, ++frontend_count);
		}
...
gboolean DeviceManager::is_frontend_supported(Dvb::Frontend const & test_frontend) {
...
Dvb::Frontend * DeviceManager::find_frontend_by_path(Glib::ustring const & path) {
	...
	for (FrontendList::iterator iterator = frontends.begin(); iterator != frontends.end(); ++iterator) {
		...
		if (current_frontend->get_path() == path) { result = current_frontend; }
	}
	return result;
}

void DeviceManager::check_frontend() {
	if (frontends.empty()) { throw Exception(_("There are no DVB devices available")); }
}
